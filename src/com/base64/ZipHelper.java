...
/**
 * <p>
 * Provides some helper methods for ZipInputStream and ZipOutputStream.
 * </p>
 * 
 * <p>
...
public final class ZipHelper {
	...
	/**
	 * This function generates the huffmann codes according to a given set of
	 * bitlegth.
	 * 
	 * @param huffmanCode
	 *            will be the resulting code pattern
	 * @param huffmanCodeLength
	 *            has to be the bitlegth for each code
	 */
	public static final void genHuffTree(int[] huffmanCode, byte[] huffmanCodeLength) {
		...
		// find the Codes for each smallest Code within all of the same
		// bitlength
	 ...
	 * 
	 * @param huffmanCode
	 *            - pointer to where the code shall be inserted
	 * @param huffmanCodeLength
	 * @param distHuffCode
	 * @param distHuffCodeLength
	 */
	public final static void genFixedTree(int[] huffmanCode, byte[] huffmanCodeLength, int[] distHuffCode, byte[] distHuffCodeLength) {
	...
	public static final void genTreeLength(int[] count, byte[] huffmanCodeLength, int max_len) {
		...
		while (true) {
			...
			// set all knot pointer of Min2 -> Min1, add one to CodeLength, and
			// remove old knots from search
		...
		// fix the overflow
		if (overflowCount != 0) {
			// System.out.println(" fixing " + overflowCount +
			// " overflows  because of max=" + max_len);
	...
	/**
	 * This function converts the representation of a huffman tree from table to
	 * tree structure.
	 * 
	 * @param huffmanCode
	 *            input
	...
	/**
	 * This function parses and pops one huffmancode out of the given buffer and
	 * returs the associated value.
	 * 
	 * @param smallCodeBuffer
	 *            the buffer that is parsed
	 * @param huffmanTree
	 *            is the representation of the tree
	 ...
	 *            the stream
	 * @throws IOException
	 *             ocurrs if the stream is not readable
	 */
	...
	/**
	 * In order to compute the CRC checksum fast it is crucial to have a
	 * precomputed table.
	 * 
	 * @param table
	 *            store the table here
	 */
	 ...
	 * 
	 * @param table
	 *            this is the precomputed crc32 table
	 * @param crc
	 *            set this value to zero if starting the computation or to the
	 *            last retrieved value when processing further data.
	 * @param buffer
	 *            the buffer to be walked
	 * @param off
	 *            the offset to start
	 * @param len
