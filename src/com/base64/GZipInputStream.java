...
public class GZipInputStream extends InputStream {
	...
	/**
	 * The data seems to be decompressed sucessfull if vaildData is true after
	 * processing the whole stream. This is determinded via counting the
	 * processed bytes and depending on the on the parameters given to the
	 * constructor also by using a CRC32 checksum.
	 */
	...
	// Buffer stuff:
	private byte[] window = new byte[32 * 1024]; // every decoder has to support
													// windows up to 32k
	...
	long[] smallCodeBuffer = new long[2];// (1) contains the merged bitcode and
											// (2) contains the count of those
											// bits
	...
	/**
	 * Creates an input stream capable of GZIP and Deflate with a buffer of 1024
	 * bytes.
	 * 
	 * @param inputStream
	 ...
	 * @param hash
	 *            set true for data checking, set false for speed reading
	 * @throws IOException
	 *             when the header of a GZIP stream cannot be skipped
	 * @see #TYPE_DEFLATE
	 * @see #TYPE_GZIP
	 */
	...
	public GZipInputStream(InputStream inputStream, int size, int compressionType, boolean hash) throws IOException {
		...
		// #debug
		// System.out.println("creating outbuff, size=" + size +
		// ", actual lenth=" + this.outBuff.length) ;
	...
	/**
	 * This function hides the fact that 'this.window' is a ring buffer so just
	 * pass 'start' and 'len' of data in the window as well as a destination and
	...
	private void copyFromWindow(int start, int len, byte[] dest, int destoff) {
		// #debug
		// System.out.println( "copyFromWindow(start=" + start + ", len=" + len
		// + ", dest.length=" + dest.length + ", destoff=" + destoff +
		// ") - window.length=" + this.window.length );
	...
	/**
	 * This function fills the internal outputbuffer reading data form the
	 * this.inputStream and inflating it.
	 */

	private void inflate() throws IOException {
		// #debug
		// System.out.println("inflate - outbuff.length=" +
		// this.outBuff.length);
		int val = 0;
			...
			if (this.status == GZipInputStream.EXPECTING_DATA) {
				...
				if (this.BTYPE == 0) {
					...
					if (this.B0len > 0) {
						...
						this.pProcessed = (this.pProcessed + copyBytes) & 32767;// %
																				// (1<<15);
					...
					if (val < 256) {
						...
						if (val > 285) {
							// ERROR: data > 285 was decoded. This is invalid
							// data.;
						...
						for (int j = 0; j < rep; j++) {
							...
							this.outEnd += cPos;
							this.pProcessed = (this.pProcessed + cPos) & 32767;// %
																				// (1<<15);
						}

						// cPos > cLen OR remainder
						copyFromWindow(aPos, rem, myOutBuff, this.outEnd);// from
																			// window
																			// into
																			// buffer,
																			// and
						...
						this.outEnd += rem;
						this.pProcessed = (this.pProcessed + rem) & 32767;// %
																			// (1<<15);
	...
	private void processHeader() throws IOException {
		...
		int[] huffmanCode = new int[286]; // this contains the codes according
											// to the huffman tree/mapping
		int[] huffmanData = new int[286]; // this contains the data referring to
											// the code.
		...
		if (this.BTYPE == 3) {
			...
			// parse the length code for the normal Tree and the distance Tree
			// using the miniTree
			...
			// this.distHuffCodeLength is read together with
			// this.huffmanCodeLength
	 ...
	 * 
	 * @return -1 if the stream is finished, 1 if the current data is valid, 0
	 *         if not
	 * @throws IOException
	 */
	 ...
	 * Therefore it will always return the buffer size that was given to the
	 * constructor.
	 */
