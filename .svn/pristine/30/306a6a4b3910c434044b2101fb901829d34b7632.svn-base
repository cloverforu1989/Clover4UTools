...
public class GZipOutputStream extends OutputStream {
	/**
	 * This constant triggers the normal deflate compression as described in rfc
	 * 1951.
	 */
	public static final int TYPE_DEFLATE = 0;
	/**
	 * This constant triggers the gzip compression that is the same as deflate
	 * with some extra header information (see rfc 1952).
	 */
	...
	private byte[] inputBuffer; // the input data is read in chunks from the
								// inputStream
	private int inEnd;
	private int inStart;

	private int[] smallCodeBuffer; // we have to buffer the output, because it
									// comes in different bitlength
	...
	private int BTYPE; // =1 for fixed Tree this happens in case of
						// huffmanwindowsize=0
	 ...
	 * 
	 * @param outputStream
	 *            stream to write the compressed data in
	 * @param size
	 *            prefered size of the internal buffer
	 ...
	 *            better compression.
	 * @throws IOException
	 *             might be thrown in case that the inputStream can not be read,
	 *             the outputStream can not be written into or in case of wrong
	 *             arguments
	 * 
	 * @see #TYPE_DEFLATE
	 * @see #TYPE_GZIP
	...
	public void close() throws IOException {
		...
		if (this.BTYPE == 2) {
			...
			// empty the huffmanwindow and force a new block on purpose, if
			// there might
			// occur a new block before the last one. This is to make sure that
			// the
	...
	/**
	 * It is strongly recomended NOT to call flush before close() since close()
	 * is able to handle the flushing better itself.
	 */
	public void flush() throws IOException {
		// flush inputBuffer -> LZ77
		LZ77(false); // do not set to true, since we still need sth. for the
						// last block
	 ...
	 * 
	 * @param bestPointer
	 *            this will hold the resulting distance and lenth pair
	 *            {distance, length}
	 * @param position
	 *            position to look at in the inputbuffer
	...
	/**
	 * This function takes the result element of a hashmap to look for previous
	 * reoccurrences of the data in the inputbuffer starting at position. The
	 * best matching distance and length pair is return in pointer.
	 * 
	 * @param found
	 *            the hashmap result of the thee chars
	 * @param pointer
	 *            distance and lenth pair {distance, length}
	 * @param position
	 *            position in the inputbuffer
	 */
	private void searchHM4LZ77(ZipIntMultShortHashMap.Element found, int[] pointer, int position) {
		int length;

		// check this.plainPointer-found0.values[k] from the end since shortest
	...
	/**
	 * This function puts the pointer into the outputwindow where it will be
	 * taken off when the huffman compression takes place.
	 * 
	 * @param distance
	 *            the distance information of the pointer
	 * @param length
	 *            the length information of the pointer
	 * @throws IOException
	 *             this might happen, when there occurr errors while writing
	...
	private void encodePointer(int distance, int length) throws IOException {
		...
		// #debug info
		// # System.out.println(length + "-Tupel found  at " + distance +
		// "back");
		...
		if (this.outputWindow.length != 0) {
			...
			this.outputWindow[this.outProcessed] = (byte) 255; // special
																// distance
																// stuff
			this.outputWindow[this.outProcessed + 1] = (byte) (litlen - 255); // 0
																				// is
																				// reserved!!

			this.outputWindow[this.outProcessed + 2] = litextra;// value of
																// extra bits
			this.outputWindow[this.outProcessed + 3] = (byte) (di);// id of
																	// Table ==
																	// code
	...
	/**
	 * This method fills a single char into the huffmanwindow or in the
	 * outputStream depending on the huffmanmode
	 * 
	 * @param position
	 *            take the char from inputbuffer at this postion
	 * @throws IOException
	 */
	...
	/**
	 * This function applies the LZ77 algorithm on the inputBuffer. The
	 * inputBuffer is cleared completely and filled into outputWindow. If
	 * outputWindow is full, compileOutput is called.
	 * 
	 * @throws IOException
	 */
	// public int l=0;
	...
	/**
	 * This function generates and writes the header for a new block. Therefore
	 * it has to calculate and compress the huffman trees.
	 * 
	 * @throws IOException
	 */
	private void newBlock() throws IOException {
		...
		if (this.BTYPE == 1) {
			...
			this.litCount[256] = 1; // theoretisch kann man es auch auf 2
									// setzen, was das Problem jedoch nicht
									// loest
	...
	/**
	 * This function applies the huffmanencoding on the collected data in
	 * outputWindow. The huffman encoded data is then written into the
	 * outputStream.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * This function compresses and writes the literal/length as well as the
	 * distance trees. At first repeatcodes are determined, then the miniTree is
	 * generated and after all they are written into the outputStream.
	 * 
	...
	private void compressTree(byte[] huffmanCodeLength, byte[] distHuffCodeLength) throws IOException {
		...
		for (int i = 0; i < HLIT; i++) {
			...
			// System.out.println("  " + j + " " + huffmanCode[j] + ", " +
			// huffmanCodeLength[j]);
			j++;
		}
		for (int i = 0; i < HDIST; i++) {
			len[j] = distHuffCodeLength[i];
			// System.out.println("  " + j + " " + distHuffCode[i] + ", " +
			// distHuffCodeLength[i]);
			j++;
		}
		...
		for (int i = 0; i < len.length; i++) {
			...
			if (i + 3 < len.length && len[i] == len[i + 1] && len[i] == len[i + 2] && len[i] == len[i + 3]) {
				if (len[i] == 0) {
					...
					// System.out.println("wrinting outCount="+outCount +
					// "   =  0");
					k = 4;

					while (i + k < len.length && len[i] == len[i + k] && k < 139) {
						k++;
					}

					if (k < 11 + 1) {
						// System.out.println("writing outCount="+outCount +
						// "   =  17");
					...
					// repeat count
					k = 4;
					while (i + k < len.length && len[i] == len[i + k] && k < 7) {
						k++;
					}
		...
		// write Mini-Tree
		for (i = 0; i < HCLEN; i++) {
			// System.out.println("" + i + "=" +miniHuffData[i] + ":\t" +
			// miniHuffCodeLength[miniHuffData[i]]);
		...
		for (i = 0; i < outCount; i++) {
			// write code
			// System.out.println("wrinting i="+i + "   =  " +
			// outLitLenDist[i]);
			...
			// System.out.println(outLitLenDist[i] + " == code: " +
			// miniHuffCode[outLitLenDist[i]]);
		}

	}

	/**
	 * This function is able to write bits into the outStream. Please mind that
	 * is uses a buffer. You might have to flush it, by giving zeros.
	 * 
	 * @param val
	 *            The bitsequence as an integer.
	 * @param len
	 *            The number of bits to process.
	 * @throws IOException
	 *             in case Errors within the outputStream ocurred
	 */
