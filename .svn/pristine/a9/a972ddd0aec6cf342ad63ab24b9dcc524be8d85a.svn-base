...
public final class ZipUtil {
	 ...
	 * @return the corresponding byte array in which integers are in the same
	 *         way encoded as in DataOutputStream.writeInt()
	 */
	...
	/**
	 * Compresses an integer RGB array without translucent parts, each RGB pixel
	 ...
	 *            the int array
	 * @param offset
	 *            the start index of the first pixel
	 * @param len
	...
	/**
	 * Converts the given int[] RGB array into a byte[] array without preserving
	 * the alpa channel (each RGB pixel is in the format 0xAARRGGBB).
	 * 
	 * @param rgb
	 *            the original integer array
	 ...
	 * 
	 * @param data
	 *            the byte array with bytes in RRGGBB order (no alpha channel).
	 ...
	 * 
	 * @param data
	 *            the original byte array with bytes in RRGGBB order (no alpha
	 *            channel).
	...
	public static int[] convertByteToRgbArray(byte[] data) {
		...
		int j = 0;
		for (int i = 0; i < rgb.length; i++) {
			rgb[i] = ((data[j + 0] & 0xff) << 16) | ((data[j + 1] & 0xff) << 8)
			// | ((data[j+2] & 0xff ) << 0);
					| (data[j + 2] & 0xff);
			j += 3;
		}
